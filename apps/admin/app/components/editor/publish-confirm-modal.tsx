...
export function PublishConfirmModal({ isOpen, onConfirm, onCancel, seoData }: PublishConfirmProps) {
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto">
        <div className="px-6 py-4 border-b border-gray-100">
          ...
          <p className="text-xs text-gray-400 mt-0.5">아래 정보로 페이지가 배포됩니다</p>
        </div>

        <div className="p-6 space-y-5">
          {/* Page info */}
          <div>
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">페이지 정보</p>
            <div className="space-y-2 text-sm">
              <div className="flex items-start gap-3">
                <span className="text-gray-400 shrink-0 w-16">제목</span>
                <span className="text-gray-900 font-medium">{seoData.title || "제목 없음"}</span>
              </div>
              <div className="flex items-start gap-3">
                <span className="text-gray-400 shrink-0 w-16">URL</span>
                <span className="text-blue-600 font-mono text-xs">promotion.ccoshong.top/{seoData.slug}</span>
              </div>
            </div>
          </div>
          ...
          {/* SEO status */}
          <div>
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">SEO 설정 상태</p>
          ...
          {/* SNS share preview */}
          <div>
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">공유 미리보기</p>
            <div className="bg-gray-50 rounded-xl border border-gray-100 overflow-hidden max-w-[340px] mx-auto">
              ...
              <div className="p-3">
                ...
                <p className="text-sm font-semibold text-gray-900 mt-0.5 line-clamp-2">{displayTitle}</p>
                {seoData.seoDescription && (
                  <p className="text-xs text-gray-500 mt-0.5 line-clamp-2">{seoData.seoDescription}</p>
                )}
              </div>
            </div>
          </div>
...
function SeoStatusRow({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="flex items-center gap-2 text-sm">
      ...
      {filled && <span className="text-xs text-gray-400 truncate max-w-[200px] ml-auto">{value}</span>}
